...
searchBtn.addEventListener("click", () => {
  ...
  fetch(url_f, options)
    ...
    .then((jsonData) => {
      // Suppose `jsonData` is your fetched forecast API result
      ...
      // Remove today's (first) and keep next 4
      let next_4_days = forecast_per_day.slice(1, 5);
      console.log(next_4_days);
      next_4_days.forEach((day, index) => {
        console.log(`Day ${index + 1}:`, day);
        // update your HTML elements here, e.g.:
        document.getElementById(`card_${index + 1}_date`).textContent =
          day.date;
        document.getElementById(`card_${index + 1}_temp`).textContent =
          day.temp;
        document.getElementById(`card_${index + 1}_wind`).textContent =
          day.wind;
        document.getElementById(`card_${index + 1}_humID`).textContent =
          day.hum;
